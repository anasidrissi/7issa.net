...
<?php
$hissa_donne=$hissa_db->query("SELECT COUNT(id) FROM hissa_beta WHERE univer_name='Fsjes Mohammedia'");
...
<nav> <!-- rit couverture -->
	<img class="img_couv" src="../img/ECONOMIE.png" /> <!-- l'image du couverture RIT -->
	<div class="about_admin">
		<span title="A propos au l'administarteur" style="color:#444" id="fs1" aria-hidden="true" data-icon="&#xe08a;"></span><a class="apropos_admin" href="#">A propos au l'administrateur</a>
		<span title="Chaine youtube" style="color:#C41A1E;" id="fs1" aria-hidden="true" data-icon="&#xe0a3;"></span><a class="youtube" href="#">Chaine youtube</a>
		<span title="La page facebook" style="color:#3974B1;" id="fs1" aria-hidden="true" data-icon="&#xe093;"></span><a class="facebook" href="#">La page facebook</a>
...
<?php
$hissa_donne=$hissa_db->query("SELECT * FROM hissa_beta WHERE univer_name='Fsjes Mohammedia' ORDER BY date DESC $limit");
while($hissa_affiche=$hissa_donne->fetch()){
	//fonction link generator
if ($hissa_affiche['link_var']=='load' OR $hissa_affiche['link_var']=="no") {
	$link_target="../cours/?cours=".(int) $hissa_affiche['id'];
}
else{
	$link_target='../hissa_play.php?v_id=' . $hissa_affiche['id'] . '&nom=' . $hissa_affiche['name'] . '&mat=' . $hissa_affiche['mat_name'];
}
	...
	echo '<a href="' . $link_target . '"><div id="hissa_index">
